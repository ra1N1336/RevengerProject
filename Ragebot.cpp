...
void CRageBot::DoAntiAim() {
	...
	if (Settings.GetBoolSetting(Tab_Ragebot, AntiAim_Enable))
	{
		...
		case 15: // Ultra (Test by DiPtoS)
		{
			...
			int value = ServerTime % 69;
			switch (value) {
			case 0:views.x = -3240.443562f;
				if (Hacks.SendPacket)
					views.x = 52.23433211f; break;
			case 1:views.x = -53.453455f;
				if (Hacks.SendPacket)
					views.x = 0.43443f; break;
			case 2:views.x = 56.2123213245999f;
				if (Hacks.SendPacket)
					views.x = -30.32335533f; break;
			case 3:views.x = -3240.21466777f;
				if (Hacks.SendPacket)
					views.x = 14.2435478999f; break;
			case 4:views.x = 1111.5446433f;
				if (Hacks.SendPacket)
					views.x = -96.45666f; break;
			case 5:views.x = -61.4354666f;
				if (Hacks.SendPacket)
					views.x = 79.f; break;
			case 6:views.x = 50.3355569f;
				if (Hacks.SendPacket)
					views.x = -46.46335324324f; break;
			case 7:views.x = -73.3243243243f;
				if (Hacks.SendPacket)
					views.x = 9.54534543f; break;
			case 8:views.x = 53.567476756f;
				if (Hacks.SendPacket)
					views.x = -69.576575675756f; break;
			case 9:views.x = 8442.43543543534f;
				if (Hacks.SendPacket)
					views.x = -3240.345345345f; break;
			case 10:views.x = -95.4535435345f;
				if (Hacks.SendPacket)
					views.x = 362.864351354545345f; break;
			case 11:views.x = 774.324234342f;
				if (Hacks.SendPacket)
					views.x = -784.324234234f; break;
			case 12:views.x = -494.98978797f;
				if (Hacks.SendPacket)
					views.x = 704.43434599f; break;
			case 13:views.x = 683.994599f;
				if (Hacks.SendPacket)
					views.x = -333.9999f; break;
			case 14:views.x = -6213.993131999999f;
				if (Hacks.SendPacket)
					views.x = 8313.9999f; break;
			case 15:views.x = 7316.6731545635f;
				if (Hacks.SendPacket)
					views.x = -5316.f; break;
			case 16:views.x = -3242.54654645f;
				if (Hacks.SendPacket)
					views.x = 742.3343f; break;
			case 17:views.x = 85.456546456f;
				if (Hacks.SendPacket)
					views.x = -554.46546436f; break;
			case 18:views.x = -85.546456456f;
				if (Hacks.SendPacket)
					views.x = 704.99343f; break;
			case 19:views.x = 71.423432423f;
				if (Hacks.SendPacket)
					views.x = -624.654654f; break;
			case 20:views.x = -735.975466f;
				if (Hacks.SendPacket)
					views.x = 353.2242424f; break;
			case 21:views.x = 351.99394f;
				if (Hacks.SendPacket)
					views.x = -953.493493943f; break;
			case 22:views.x = -79.5464535455643f;
				if (Hacks.SendPacket)
					views.x = 69.945945945f; break;
			case 23:views.x = 69.32325325f;
				if (Hacks.SendPacket)
					views.x = -70.423432455f; break;
			case 24:views.x = -74.98897f;
				if (Hacks.SendPacket)
					views.x = 313.3223144f; break;
			case 25:views.x = 793.1554433f;
				if (Hacks.SendPacket)
					views.x = -795.2432423f; break;
			case 26:views.x = -791.23423423f;
				if (Hacks.SendPacket)
					views.x = 491.9978f; break;
			case 27:views.x = 88.9999f;
				if (Hacks.SendPacket)
					views.x = -581.323232411f; break;
			case 28:views.x = 58.939243294f;
				if (Hacks.SendPacket)
					views.x = -950.9995f; break;
			case 29:views.x = 81.9928393f;
				if (Hacks.SendPacket)
					views.x = -616.9999f; break;
			case 30:views.x = -921.2145423f;
				if (Hacks.SendPacket)
					views.x = 531.93948343f; break;
			}
			counter++;
		}
		break;
		...
		{
			if (Hacks.SendPacket)
				Fast_Spin_Yaw += 3.9932;
			float add = totarget.y * 1.99005 + Fast_Spin_Yaw;
			if (!Hacks.SendPacket)
				add += 88.5722432344;
			views.y += add;
		}
		...
		case 8:
		{
			if (!Settings.GetSetting(Tab_Misc, Misc_Anti_Untrust)) {
				if (Hacks.LocalPlayer->GetVecVelocity().Length2D() > 300)
				{
					...
					views.y -= 84.9999055523;
				}
			}
			else {
				if (Hacks.CurrentCmd->buttons & IN_ATTACK) {
					views.y += 87.656645363;
				}
				else
					views.y += 174.4788437875;
			}
		}
		break;
		...
		case 14:
		{
			views.y = 130.88345156;
			break;
		}
		case 15:
		{
			views.y = 110.93756456;
			break;
		}
		case 16:
		{
