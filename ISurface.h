...
#pragma once
	...
	void DrawString(DWORD font, int x, int y, Color color, bool bCenter, const char* msg, ...)
	{
		...
		Interfaces.pSurface->GetTextSize(font, wbuf, iWidth, iHeight);
		Interfaces.pSurface->DrawSetTextFont(font);
		Interfaces.pSurface->DrawSetTextColor(r, g, b, a);
		Interfaces.pSurface->DrawSetTextPos(!bCenter ? x : x - iWidth / 2, y - iHeight / 2);
	...
	void Text2(int x, int y, Color color, DWORD font, const char* text);
